...
pub async fn install_artifacts(config: &Config) -> Result<()> {
    ...
    // - Uses the host's dracut to build a proper initramfs
...
/// Set SELinux contexts on binaries and libraries to allow execution and loading.
/// On SELinux-enabled systems, files copied to /opt/kata get the var_t context,
/// which doesn't allow execution. We change binaries to bin_t (standard binary type)
/// and libraries to lib_t (standard library type).
fn set_selinux_contexts(dir: &str) -> Result<()> {
    use std::process::Command;

    // In DaemonSet mode, we run in a container with /host mounted.
    // We need to use the host's chcon binary to set SELinux labels properly.
    let host_chcon = Path::new("/host/usr/bin/chcon");
    if !host_chcon.exists() {
        log::debug!("Host chcon not found at {:?}, skipping SELinux context setup", host_chcon);
        return Ok(());
    }

    // Directories containing executables that need bin_t context
    let executable_dirs = ["bin", "libexec", "runtime-rs/bin"];

    for subdir in executable_dirs.iter() {
        let full_path = Path::new(dir).join(subdir);
        if !full_path.exists() {
            continue;
        }
        ...
        for entry in fs::read_dir(&full_path)? {
            ...
            let path = entry.path();
            if path.is_file() {
                // Set SELinux type to bin_t (standard executable type)
                // Use the host's chcon binary with the full /host path
                let status = Command::new(host_chcon)
                    .args(["-t", "bin_t"])
                    .arg(&path)
                    .status();
                ...
                match status {
                    Ok(s) if s.success() => {
                        log::debug!("Set SELinux context bin_t on {:?}", path);
                    }
                    Ok(s) => {
                        log::warn!("chcon failed for {:?} with exit code {:?}", path, s.code());
                    }
                    Err(e) => {
                        log::warn!("Failed to run chcon for {:?}: {}", path, e);
                    }
                }
            }
        }
    ...
    // Directories containing shared libraries that need lib_t context
    // Without this, binaries with RPATH pointing to lib64 can't load libraries
    let library_dirs = ["lib64"];

    for subdir in library_dirs.iter() {
        let full_path = Path::new(dir).join(subdir);
        if !full_path.exists() {
            continue;
        }

        // Set lib_t context recursively on the lib64 directory
        let status = Command::new(host_chcon)
            .args(["-R", "-t", "lib_t"])
            .arg(&full_path)
            .status();
        ...
        match status {
            Ok(s) if s.success() => {
                log::debug!("Set SELinux context lib_t on {:?}", full_path);
            }
            Ok(s) => {
                log::warn!("chcon failed for {:?} with exit code {:?}", full_path, s.code());
            }
            Err(e) => {
                log::warn!("Failed to run chcon for {:?}: {}", full_path, e);
            }
        }
    }

    info!("SELinux contexts set on kata binaries");
    Ok(())
}

/// Build initrd at runtime using kata-osbuilder.sh from the downstream kata-containers RPM.
...
fn build_initrd_with_modules(dir: &str) -> Result<()> {
    ...
    let chmod_status = Command::new("chmod")
        .args(["+x", osbuilder_script.to_str().unwrap()])
        .status()
        .context("Failed to chmod osbuilder script")?;
    ...
    // - Use systemd as init (not busybox)
    // - Include all necessary drivers (vsock, virtio_console, virtiofs, etc.)
    ...
    let status = Command::new("/host/usr/bin/nsenter")
        .args(&osbuilder_args)
        .status()
        .context("Failed to run kata-osbuilder.sh via nsenter")?;

    if !status.success() {
        return Err(anyhow::anyhow!(
            "kata-osbuilder.sh failed with exit code: {:?}. Check that dracut is available on the host.",
            status.code()
        ));
    }

    // kata-osbuilder.sh creates symlinks in /var/cache/kata-containers/ on the host:
    // - vmlinuz.container -> /lib/modules/<kernel>/vmlinuz
    // - kata-containers-initrd.img -> osbuilder-images/<kernel>/<distro>-kata-<kernel>.initrd
    //
    // The configuration files point directly to /var/cache/kata-containers/ so we don't
    // need to create additional symlinks. We trust the osbuilder's exit code to indicate
    // success - if it returned 0, the symlinks were created.
    //
    // Note: We don't check file existence here because of mount namespace visibility issues
    // between the container (where this code runs) and the host (where osbuilder creates files).
    // The /host bind mount may not immediately reflect changes made in the host namespace.
